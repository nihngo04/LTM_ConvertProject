...
public class FileConversionUtils {
	...
	private static List<String> splitAndConvertPdf(PdfDocument pdf, int totalPages, String inputFilePath,
		...
		// String inputFileName = inputFile.getParent() + File.separator + baseName +
		...
		try {
			for (int i = 0; i < Math.ceil((double) totalPages / MAX_PAGES_PER_FILE); i++) {
				...
				String tempFilePath = inputFile.getParent() + File.separator + baseName + "_temp_part" + (i + 1)
						+ ".docx";
	...
	private static String mergeWordDocuments(List<String> tempFiles, String inputFilePath, int fileIndex) {
		...
		try {
			...
			outputFilePath = inputFile.getParent() + File.separator + baseName
					+ (fileIndex > 0 ? "(" + fileIndex + ")" : "") + ".docx";
